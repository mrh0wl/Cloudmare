...
# In case you cannot install some of the requiR development packages
# there's also an option to disable the SSL warning:
try:
    import thirdparty.requests.packages.urllib3
    thirdparty.requests.packages.urllib3.disable_warnings()
except Exception:
    pass
...
def parse_args():
    ...
    parser.add_argument('-d', '--domain', help="Domain name to enumerate it's subdomains", requiR=True)
    parser.add_argument('-b', '--bruteforce', help='Enable the subbrute bruteforce module', nargs='?', default=False)
    parser.add_argument('-p', '--ports', help='Scan the found subdomains against specified tcp ports')
    parser.add_argument('-v', '--verbose', help='Enable Verbosity and display results in realtime',
                        nargs='?', default=False)
    parser.add_argument('-t', '--threads', help='Number of threads to use for subbrute bruteforce',
                        type=int, default=30)
...
def write_file(filename, subdomains):
    ...
    print("%s%sSaving results to file: %s%s%s%s" % (tab, info, W, R, filename, W))
...
class enumratorBase(object):
    ...
    def print_banner(self):
        ...
        # self.print_(tab + run + "Searching now in %s.." % (self.engine_name) + W)
        return
...
class GoogleEnum(enumratorBaseThreaded):
    def __init__(self, domain, subdomains=None, q=None, silent=False, verbose=True):
        ...
        self.MAX_DOMAINS = 11
        self.MAX_PAGES = 200
        super(GoogleEnum, self).__init__(base_url,
                                         self.engine_name,
                                         domain, subdomains,
        ...
        try:
            ...
            for link in links_list:
                ...
                if subdomain and subdomain not in self.subdomains and subdomain != self.domain:
                    if self.verbose:
                        self.print_(tab*2 + "%s%s: %s%s" % (R, self.engine_name, W, subdomain))
    ...
    def check_response_errors(self, resp):
        if (type(resp) is str) and 'Our systems have detected unusual traffic' in resp:
            self.print_(bad + R + "Google probably now is blocking our requests" + W)
            self.print_(run + "Finished now the Google Enumeration ..." + W)
            return False
        return True
...
class AskEnum(enumratorBaseThreaded):
    def __init__(self, domain, subdomains=None, q=None, silent=False, verbose=True):
        ...
        base_url = 'http://www.ask.com/web?q={query}&page={page_no}\
            &qid=8D6EE6BF52E0C04527E51F64F22C4534&o=0&l=dir&qsrc=998&qo=pagination'
...
class DNSdumpster(enumratorBaseThreaded):
    ...
    def check_host(self, host):
        ...
        try:
            ...
            if ip:
                if self.verbose:
                    self.print_(tab*2 + "%s%s: %s%s" % (R, self.engine_name, W, host))
                is_valid = True
...
parser.set('VTOTAL', 'api_key',
                """
                end_cmd = """)
with open('data/APIs/api.conf', 'w') as configfile:
    parser.write(configfile)"""
                quest(f"{warn}Virustotal API key not found! Do you want to enter your API key?: ",
                      cmd + f"input(\"{tab}{warn}\" + \"Virustotal API key: \")" + end_cmd,
                      cmd + "'noapi'" + end_cmd,
                      "no",
                      **{"printSuccess": "{good}API key saved.",
                         "printError": "{bad}API key not saved.",
                         }
                      )
            apiHeads = {
    ...
    def enumerate(self):
        while self.url != '':
            ...
            if 'error' in resp and self.apikey != "":
                self.print_(
                    f"{bad}{R}Virustotal probably now is blocking our requests! Or you have not a valid api key!{W}"
                )
                break
...
def main(domain, threads, savefile, ports, silent, verbose, enable_bruteforce, engines):
    ...
    if not silent:
        print(info + "Enumerating subdomains now for %s" % parsed_domain.netloc + W)

    if verbose and not silent:
        print(tab + warn + "verbosity is enabled, will show the subdomains results in realtime" + W)
    ...
    if enable_bruteforce:
        ...
        output = False
        json_output = False
        bruteforce_list = subbrute.print_target(parsed_domain.netloc, record_type,
                                                subs, resolvers, process_count, output,
                                                json_output, search_list, verbose)

    subdomains = search_list.union(bruteforce_list)

    # blacklist of domains
    blacklistedDomainsFile = open("data/input/blacklistdomains", "r")
        ...
        for bsubdomain in blacklistEntry:
            for wsubdomain in subdomains:
                if str(wsubdomain).startswith(str(bsubdomain.replace("\n", ""))):
                    ...
                    print(tab*2 + warn + "Removing "+wsubdomain+" "+f"({R}blacklisted{W})")

        if savefile:
            write_file(savefile, subdomains)

        if not silent:
            print(tab + warn + "Total Unique Subdomains Found: %s%s" % (Y, len(subdomains)) + W)

        if ports:
            if not silent:
                print(tab + warn + "Start port scan now for the following ports: %s%s" % (Y, ports) + W)
            ports = ports.split(',')
            ...
            for subdomain in subdomains:
                try:
                    ...
                    if isCloud is None:
                        ...
                        print(resultPrint + isCloud)
                except Exception:
                    print(tab*2 + bad + subdomain + f" ({R}error getting ip{W})")

    print(info + 'Enumerating misconfigured subdomains')
    subdomains = [misconfigured for misconfigured in subdomains if ISPCheck(misconfigured) is None]

    print(tab + warn + 'Total Misconfigured Subdomains Found: %s' % len(subdomains))
    if len(subdomains) == 0:
        print(tab*2 + bad + 'misconfigured subdomains not found')
    else:
        for subdomain in subdomains:
            print(tab*2 + good + subdomain)
...
def interactive():
    ...
    if args.no_color:
        no_color()
    banner()
    main(domain, threads, savefile, ports, silent=False, verbose=verbose,
