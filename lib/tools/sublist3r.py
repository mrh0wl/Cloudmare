...
from lib.parse.colors import W, R, G, Y, info, que, bad, good, run, tab
...
class GoogleEnum(enumratorBaseThreaded):
	...
	def check_response_errors(self, resp):
		if (type(resp) is str or type(resp) is unicode) and 'Our systems have detected unusual traffic' in resp:
			self.print_(bad + R + "Error: Google probably now is blocking our requests" + W)
...
def main(domain, threads, savefile, ports, silent, verbose, enable_bruteforce, engines):
	...
	if not silent:
		print(que + "Enumerating subdomains now for %s" % parsed_domain.netloc + W)

	if verbose and not silent:
		print(tab + info + "verbosity is enabled, will show the subdomains results in realtime" + W)
	...
	if enable_bruteforce:
		if not silent:
			print(tab + info + "Starting bruteforce module now using subbrute.." + W)
		record_type = False
	...
	if subdomains:
		...
		if not silent:
			print(tab + info + "Total Unique Subdomains Found: %s%s" % (Y, len(subdomains)) + W)

		if ports:
			if not silent:
				print(tab + info + "Start port scan now for the following ports: %s%s" % (Y, ports) + W)
			ports = ports.split(',')
	...
	print(que + 'Enumerating misconfigured subdomains')
	subdomains = [misconfigured for misconfigured in subdomains if ISPCheck(misconfigured) == None]
	
	print(tab + info + 'Total Misconfigured Subdomains Found: %s' % len(subdomains))
	if len(subdomains) == 0:
