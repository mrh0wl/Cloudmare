...
from lib.utils.colors import W, R, G, Y, info, warn, bad, good, run, tab
    ...
    def enumerate(self):
        while self.url != '':
            ...
            if 'error' in resp and self.apikey != "":
                self.print_(f"{bad}{R}Virustotal probably now is blocking our requests! Or you have not a valid api key!{W}")
                break
