...
from thirdparty.dns.query import _compute_times, _matches_destination, BadResponse, ssl
...
async def send_udp(sock, what, destination, expiration=None):
    ...
    *sock*, a ``thirdparty.dns.asyncbackend.DatagramSocket``.
...
async def receive_udp(sock, destination=None, expiration=None,
    ...
    *raise_on_truncation*, a ``bool``.  If ``True``, raise an exception if
    ...
    Returns a ``(thirdparty.dns.message.Message, float, tuple)`` tuple of the received
...
async def udp(q, where, timeout=None, port=53, source=None, source_port=0,
    ...
    *sock*, a ``thirdparty.dns.asyncbackend.DatagramSocket``, or ``None``,
...
async def udp_with_fallback(q, where, timeout=None, port=53, source=None,
    ...
    *udp_sock*, a ``thirdparty.dns.asyncbackend.DatagramSocket``, or ``None``,
    the socket to use for the UDP query.  If ``None``, the default, a
    socket is created.  Note that if a socket is provided the *source*,
    *source_port*, and *backend* are ignored for the UDP query.

    *tcp_sock*, a ``thirdparty.dns.asyncbackend.StreamSocket``, or ``None``, the
...
async def tcp(q, where, timeout=None, port=53, source=None, source_port=0,
    ...
    *sock*, a ``thirdparty.dns.asyncbacket.StreamSocket``, or ``None``, the
