...
def make_ds(name, key, algorithm, origin=None):
    if algorithm.upper() == 'SHA1':
        dsalg = 1
        hash = thirdparty.dns.hash.hashes['SHA1']()
    elif algorithm.upper() == 'SHA256':
        dsalg = 2
        hash = thirdparty.dns.hash.hashes['SHA256']()
    else:
...
def _make_hash(algorithm):
    if _is_md5(algorithm):
        return thirdparty.dns.hash.hashes['MD5']()
    if _is_sha1(algorithm):
        return thirdparty.dns.hash.hashes['SHA1']()
    if _is_sha256(algorithm):
        return thirdparty.dns.hash.hashes['SHA256']()
    if _is_sha384(algorithm):
        return thirdparty.dns.hash.hashes['SHA384']()
    if _is_sha512(algorithm):
        return thirdparty.dns.hash.hashes['SHA512']()
...
def _validate_rrsig(rrset, rrsig, keys, origin=None, now=None):
    ...
    @type rrsig: thirdparty.dns.rrset.Rdata
