...
class UnsupportedAlgorithm(thirdparty.dns.exception.DNSException):
    """The DNSSEC algorithm is not supported."""


class ValidationFailure(thirdparty.dns.exception.DNSException):
    """The DNSSEC signature is invalid."""


class Algorithm(thirdparty.dns.enum.IntEnum):
    RSAMD5 = 1
    DH = 2
    DSA = 3
    ECC = 4
...
def key_id(key):
    ...
    *key*, a ``thirdparty.dns.rdtypes.ANY.DNSKEY.DNSKEY``
...
class DSDigest(thirdparty.dns.enum.IntEnum):
...
def make_ds(name, key, algorithm, origin=None):
    ...
    *name*, a ``thirdparty.dns.name.Name`` or ``str``, the owner name of the DS record.

    *key*, a ``thirdparty.dns.rdtypes.ANY.DNSKEY.DNSKEY``, the key the DS is about.

    *algorithm*, a ``str`` or ``int`` specifying the hash algorithm.
    The currently supported hashes are "SHA1", "SHA256", and "SHA384". Case
    does not matter for these strings.

    *origin*, a ``thirdparty.dns.name.Name`` or ``None``.  If `key` is a relative name,
    then it will be made absolute using the specified origin.

    Raises ``UnsupportedAlgorithm`` if the algorithm is unknown.

    Returns a ``thirdparty.dns.rdtypes.ANY.DS.DS``
    """
    ...
    if isinstance(name, str):
        name = thirdparty.dns.name.from_text(name, origin)
    ...
    dsrdata = struct.pack("!HBB", key_id(key), key.algorithm, algorithm) + \
        digest
    return thirdparty.dns.rdata.from_wire(thirdparty.dns.rdataclass.IN, thirdparty.dns.rdatatype.DS, dsrdata, 0,
                               len(dsrdata))
...
def _validate_rrsig(rrset, rrsig, keys, origin=None, now=None):
    ...
    *rrsig*, a ``thirdparty.dns.rdata.Rdata``, the signature to validate.

    *keys*, the key dictionary, used to find the DNSKEY associated
    with a given name.  The dictionary is keyed by a
    ``thirdparty.dns.name.Name``, and has ``thirdparty.dns.node.Node`` or
    ``thirdparty.dns.rdataset.Rdataset`` values.

    *origin*, a ``thirdparty.dns.name.Name`` or ``None``, the origin to use for relative
    names.
    ...
    *now*, an ``int`` or ``None``, the time, in seconds since the epoch, to
    ...
    for candidate_key in candidate_keys:
        ...
        if rrsig.labels < len(rrname) - 1:
            ...
            rrname = thirdparty.dns.name.from_text('*', suffix)
...
def _validate(rrset, rrsigset, keys, origin=None, now=None):
    ...
    *origin*, a ``thirdparty.dns.name.Name``, the origin to use for relative names;
    defaults to None.
...
class NSEC3Hash(thirdparty.dns.enum.IntEnum):
...
def nsec3_hash(domain, salt, iterations, algorithm):
    ...
    *domain*, a ``thirdparty.dns.name.Name`` or ``str``, the name to hash.
    ...
    *algorithm*, a ``str`` or ``int``, the hash algorithm.
    ...
    if not isinstance(domain, thirdparty.dns.name.Name):
        domain = thirdparty.dns.name.from_text(domain)
