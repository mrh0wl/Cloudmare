...
def make_ds(name, key, algorithm, origin=None):
    if algorithm.upper() == 'SHA1':
        dsalg = 1
        hash = thirdparty.dns.hash.get('SHA1')()
    elif algorithm.upper() == 'SHA256':
        dsalg = 2
        hash = thirdparty.dns.hash.get('SHA256')()
    else:
    ...
    return dns.rdata.from_wire(thirdparty.dns.rdataclass.IN, dns.rdatatype.DS, dsrdata, 0,
                               len(dsrdata))
...
def _validate_rrsig(rrset, rrsig, keys, origin=None, now=None):
    ...
    @type rrset: dns.rrset.RRset or (thirdparty.dns.name.Name, dns.rdataset.Rdataset)
    tuple
    ...
    if isinstance(origin, (str, unicode)):
        origin = thirdparty.dns.name.from_text(origin, dns.name.root)
...
def _validate(rrset, rrsigset, keys, origin=None, now=None):
    ...
    @type rrsigset: dns.rrset.RRset or (thirdparty.dns.name.Name, dns.rdataset.Rdataset)
    tuple
    @param keys: The key dictionary.
    @type keys: a dictionary keyed by dns.name.Name with node or rdataset values
    @param origin: The origin to use for relative names
