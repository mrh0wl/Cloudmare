...
public_enum_domain = thirdparty.dns.name.from_text('e164.arpa.')
...
def from_e164(text, origin=public_enum_domain):
    ...
    *origin*, a ``thirdparty.dns.name.Name``, the domain in which the number
    ...
    parts.reverse()
    return thirdparty.dns.name.from_text('.'.join(parts), origin=origin)
...
def query(number, domains, resolver=None):
    ...
    *domains* is an iterable containing ``thirdparty.dns.name.Name`` values.

    *resolver*, a ``thirdparty.dns.resolver.Resolver``, is the resolver to use.  If
    ``None``, the default resolver is used.
    """

    if resolver is None:
        resolver = thirdparty.dns.resolver.get_default_resolver()
    e_nx = thirdparty.dns.resolver.NXDOMAIN()
    for domain in domains:
        if isinstance(domain, str):
            domain = thirdparty.dns.name.from_text(domain)
        qname = thirdparty.dns.e164.from_e164(number, domain)
        try:
