...
def inet_pton(family, text):
    ...
    *text* is a ``str``, the textual address.
    ...
    if family == AF_INET:
        return thirdparty.dns.ipv4.inet_aton(text)
    elif family == AF_INET6:
        return thirdparty.dns.ipv6.inet_aton(text, True)
    else:
...
def inet_ntop(family, address):
    ...
    *address* is a ``bytes``, the network address in binary form.
    ...
    if family == AF_INET:
        return thirdparty.dns.ipv4.inet_ntoa(address)
    elif family == AF_INET6:
        return thirdparty.dns.ipv6.inet_ntoa(address)
    else:
        raise NotImplementedError
...
def af_for_address(text):
    ...
    *text*, a ``str``, the textual address.
    ...
    try:
        ...
        try:
            thirdparty.dns.ipv6.inet_aton(text, True)
            return AF_INET6
        except Exception:
            raise ValueError
...
def is_multicast(text):
    ...
    *text*, a ``str``, the textual address.
    ...
    try:
        first = thirdparty.dns.ipv4.inet_aton(text)[0]
        return first >= 224 and first <= 239
    except Exception:
        try:
            first = thirdparty.dns.ipv6.inet_aton(text, True)[0]
            return first == 255
        except Exception:
            raise ValueError
