        ...
        if best_start == 0 and \
           (best_len == 6 or
            ...
            if best_len == 6:
                ...
                prefix = b'::ffff:'
            hex = prefix + thirdparty.dns.ipv4.inet_ntoa(address[12:])
        else:
...
def inet_aton(text):
    ...
    @type text: string
    @rtype: string
    @raises thirdparty.dns.exception.SyntaxError: the text was not properly formatted
    """
    ...
    if not m is None:
        b = bytearray(thirdparty.dns.ipv4.inet_aton(m.group(2)))
