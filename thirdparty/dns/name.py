...
@type root: thirdparty.dns.name.Name object
@var empty: The empty DNS name.
@type empty: thirdparty.dns.name.Name object
"""
...
class EmptyLabel(thirdparty.dns.exception.SyntaxError):
    """A DNS label is empty."""


class BadEscape(thirdparty.dns.exception.SyntaxError):
    """An escaped code in a text format of DNS name is invalid."""


class BadPointer(thirdparty.dns.exception.FormError):
    """A DNS compression pointer points forward instead of backward."""


class BadLabelType(thirdparty.dns.exception.FormError):
    """The label type in DNS name wire format is unknown."""


class NeedAbsoluteNameOrOrigin(thirdparty.dns.exception.DNSException):
    """An attempt was made to convert a non-absolute name to
    wire when there was also a non-absolute (or missing) origin."""


class NameTooLong(thirdparty.dns.exception.FormError):
    """A DNS name is > 255 octets long."""


class LabelTooLong(thirdparty.dns.exception.SyntaxError):
    """A DNS label is > 63 octets long."""


class AbsoluteConcatenation(thirdparty.dns.exception.DNSException):
    """An attempt was made to append anything other than the
    empty name to an absolute DNS name."""


class NoParent(thirdparty.dns.exception.DNSException):
    """An attempt was made to get the parent of the root name
    or the empty name."""

class NoIDNA2008(thirdparty.dns.exception.DNSException):
    """IDNA 2008 processing was requested but the idna module is not
    available."""


class IDNAException(thirdparty.dns.exception.DNSException):
...
def _validate_labels(labels):
    ...
    Raises ``thirdparty.dns.name.NameTooLong`` if the name as a whole is too long.
    Raises ``thirdparty.dns.name.EmptyLabel`` if a label is empty (i.e. the root
...
class Name(object):
    ...
    """A DNS name.
    The thirdparty.dns.name.Name class represents a DNS name as a tuple of
    ...
    def fullcompare(self, other):
        ...
        *relation* describes the relation ship between the names,
        and is one of: ``thirdparty.dns.name.NAMERELN_NONE``,
        ``thirdparty.dns.name.NAMERELN_SUPERDOMAIN``, ``thirdparty.dns.name.NAMERELN_SUBDOMAIN``,
        ``thirdparty.dns.name.NAMERELN_EQUAL``, or ``thirdparty.dns.name.NAMERELN_COMMONANCESTOR``.
    ...
    def to_unicode(self, omit_final_dot=False, idna_codec=None):
        ...
        *idna_codec* specifies the IDNA encoder/decoder.  If None, the
        thirdparty.dns.name.IDNA_2003_Practical encoder/decoder is used.
    ...
    def to_digestable(self, origin=None):
        ...
        *origin* is a ``thirdparty.dns.name.Name`` or ``None``.  If the name is
        relative and origin is not ``None``, then origin will be appended
        to the name.
        Raises ``thirdparty.dns.name.NeedAbsoluteNameOrOrigin`` if the name is
    ...
    def split(self, depth):
        ...
        *depth* is an ``int`` specifying the number of labels in the suffix
        ...
        l = len(self.labels)
        if depth == 0:
            return (self, thirdparty.dns.name.empty)
        elif depth == l:
            return (thirdparty.dns.name.empty, self)
    ...
    def concatenate(self, other):
        ...
        Raises ``thirdparty.dns.name.AbsoluteConcatenation`` if the name is
    ...
    def parent(self):
        ...
        Raises ``thirdparty.dns.name.NoParent`` if the name is either the root name or the
...
def from_unicode(text, origin=root, idna_codec=None):
    ...
    *origin*, a ``thirdparty.dns.name.Name``, specifies the origin to
...
def from_wire(message, current):
    ...
    *current*, an ``int``, is the offset of the beginning of the name
    ...
    Raises ``thirdparty.dns.name.BadPointer`` if a compression pointer did not
    point backwards in the message.
    Raises ``thirdparty.dns.name.BadLabelType`` if an invalid label type was encountered.
    Returns a ``(thirdparty.dns.name.Name, int)`` tuple consisting of the name
    ...
    message = thirdparty.dns.wiredata.maybe_wrap(message)
    labels = []
