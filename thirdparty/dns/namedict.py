...
class NameDict(MutableMapping):
    """A dictionary whose keys are thirdparty.dns.name.Name objects.
    ...
    def __setitem__(self, key, value):
        if not isinstance(key, thirdparty.dns.name.Name):
    ...
    def get_deepest_match(self, name):
        ...
        *name*, a ``thirdparty.dns.name.Name``, the name to find.

        Returns a ``(key, value)`` where *key* is the deepest
        ``thirdparty.dns.name.Name``, and *value* is the value associated with *key*.
        """
        ...
        for i in range(-depth, 0):
            n = thirdparty.dns.name.Name(name[i:])
            if n in self:
                return (n, self[n])
        v = self[thirdparty.dns.name.empty]
        return (thirdparty.dns.name.empty, v)
