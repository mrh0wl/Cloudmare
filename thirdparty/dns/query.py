...
class UnexpectedSource(thirdparty.dns.exception.DNSException):
    """A DNS query response came from an unexpected address or port."""


class BadResponse(thirdparty.dns.exception.FormError):
    """A DNS query response does not respond to the question asked."""


class TransferError(thirdparty.dns.exception.DNSException):
    """A zone transfer response got a non-zero rcode."""

    def __init__(self, rcode):
        message = 'Zone transfer error: %s' % thirdparty.dns.rcode.to_text(rcode)
        super().__init__(message)
        self.rcode = rcode


class NoDOH(thirdparty.dns.exception.DNSException):
    """DNS over HTTPS (DOH) was requested but the thirdparty.requests module is not
    available."""
...
def _wait_for(fd, readable, writable, error, expiration):
    ...
    while not done:
        if expiration is None:
            ...
            if timeout <= 0.0:
                raise thirdparty.dns.exception.Timeout
        try:
...
def _addresses_equal(af, a1, a2):
    ...
    try:
        n1 = thirdparty.dns.inet.inet_pton(af, a1[0])
        n2 = thirdparty.dns.inet.inet_pton(af, a2[0])
...
def _matches_destination(af, from_address, destination, ignore_unexpected):
    ...
    if _addresses_equal(af, from_address, destination) or \
       (thirdparty.dns.inet.is_multicast(destination[0]) and
...
def _destination_and_source(where, port, source, source_port,
    ...
    destination = None
    try:
        af = thirdparty.dns.inet.af_for_address(where)
        destination = where
    except Exception:
    ...
    if source:
        saf = thirdparty.dns.inet.af_for_address(source)
        if af:
    ...
    # tuples.
    if destination:
        destination = thirdparty.dns.inet.low_level_address_tuple((destination, port), af)
    if source:
        source = thirdparty.dns.inet.low_level_address_tuple((source, source_port), af)
...
def https(q, where, timeout=None, port=443, source=None, source_port=0,
    ...
    *q*, a ``thirdparty.dns.message.Message``, the query to send.
    ...
    *session*, a ``thirdparty.requests.session.Session``.  If provided, the session to use
    to send the queries.
    ...
    *verify*, a ``str``, containing a path to a certificate file or directory.
    ...
    try:
        where_af = thirdparty.dns.inet.af_for_address(where)
    ...
    with contextlib.ExitStack() as stack:
        if not session:
            session = stack.enter_context(thirdparty.requests.sessions.Session())
    ...
    if response.status_code < 200 or response.status_code > 299:
        raise ValueError('{} responded with status code {}'
                         '\nResponse body: {}'.format(where,
                                                      ...
                                                      response.content))
    r = thirdparty.dns.message.from_wire(response.content,
                              keyring=q.keyring,
...
def send_udp(sock, what, destination, expiration=None):
    ...
    *what*, a ``bytes`` or ``thirdparty.dns.message.Message``, the message to send.
    ...
    *expiration*, a ``float`` or ``None``, the absolute time at which
    ...
    if isinstance(what, thirdparty.dns.message.Message):
        what = what.to_wire()
...
def receive_udp(sock, destination=None, expiration=None,
    ...
    *raise_on_truncation*, a ``bool``.  If ``True``, raise an exception if
    ...
    If *destination* is not ``None``, returns a ``(thirdparty.dns.message.Message, float)``
    tuple of the received message and the received time.

    If *destination* is ``None``, returns a
    ``(thirdparty.dns.message.Message, float, tuple)``
    ...
    r = thirdparty.dns.message.from_wire(wire, keyring=keyring, request_mac=request_mac,
                              one_rr_per_rrset=one_rr_per_rrset,
                              ignore_trailing=ignore_trailing,
...
def udp_with_fallback(q, where, timeout=None, port=53, source=None,
    ...
    *tcp_sock*, a ``socket.socket``, or ``None``, the socket to use for the
    ...
    Returns a (``thirdparty.dns.message.Message``, tcp) tuple where tcp is ``True``
...
def receive_tcp(sock, expiration=None, one_rr_per_rrset=False,
    ...
    *ignore_trailing*, a ``bool``.  If ``True``, ignore trailing
    ...
    Returns a ``(thirdparty.dns.message.Message, float)`` tuple of the received message
...
def xfr(where, zone, rdtype=thirdparty.dns.rdatatype.AXFR, rdclass=thirdparty.dns.rdataclass.IN,
        timeout=None, port=53, keyring=None, keyname=None, relativize=True,
        lifetime=None, source=None, source_port=0, serial=0,
        use_udp=False, keyalgorithm=thirdparty.dns.tsig.default_algorithm):
    """Return a generator for the responses to a zone transfer.

    *where*, a ``str`` containing an IPv4 or IPv6 address,  where
    to send the message.

    *zone*, a ``thirdparty.dns.name.Name`` or ``str``, the name of the zone to transfer.

    *rdtype*, an ``int`` or ``str``, the type of zone transfer.  The
    default is ``thirdparty.dns.rdatatype.AXFR``.  ``thirdparty.dns.rdatatype.IXFR`` can be
    used to do an incremental transfer instead.

    *rdclass*, an ``int`` or ``str``, the class of the zone transfer.
    The default is ``thirdparty.dns.rdataclass.IN``.
    ...
    *keyname*, a ``thirdparty.dns.name.Name`` or ``str``, the name of the TSIG
    key to use.

    *relativize*, a ``bool``.  If ``True``, all names in the zone will be
    relativized to the zone origin.  It is essential that the
    relativize setting matches the one specified to
    ``thirdparty.dns.zone.from_xfr()`` if using this generator to make a zone.
    ...
    *serial*, an ``int``, the SOA serial number to use as the base for
    ...
    ``thirdparty.dns.rdatatype.IXFR``).

    *use_udp*, a ``bool``.  If ``True``, use UDP (only meaningful for IXFR).

    *keyalgorithm*, a ``thirdparty.dns.name.Name`` or ``str``, the TSIG algorithm to use.

    Raises on errors, and so does the generator.

    Returns a generator of ``thirdparty.dns.message.Message`` objects.
    """

    if isinstance(zone, str):
        zone = thirdparty.dns.name.from_text(zone)
    rdtype = thirdparty.dns.rdatatype.RdataType.make(rdtype)
    q = thirdparty.dns.message.make_query(zone, rdtype, rdclass)
    if rdtype == thirdparty.dns.rdatatype.IXFR:
        rrset = thirdparty.dns.rrset.from_text(zone, 0, 'IN', 'SOA',
                                    '. . %u 0 0 0 0' % serial)
    ...
    (af, destination, source) = _destination_and_source(where, port,
                                                        source, source_port)
    if use_udp and rdtype != thirdparty.dns.rdatatype.IXFR:
    ...
    with _make_socket(af, sock_type, source) as s:
        ...
        if relativize:
            origin = zone
            oname = thirdparty.dns.name.empty
        else:
            origin = None
            oname = zone
        tsig_ctx = None
        while not done:
            ...
            is_ixfr = (rdtype == thirdparty.dns.rdatatype.IXFR)
            r = thirdparty.dns.message.from_wire(wire, keyring=q.keyring,
            ...
            if soa_rrset is None:
                ...
                rrset = r.answer[0]
                if rrset.rdtype != thirdparty.dns.rdatatype.SOA:
                    raise thirdparty.dns.exception.FormError("first RRset is not an SOA")
                answer_index = 1
                soa_rrset = rrset.copy()
                if rdtype == thirdparty.dns.rdatatype.IXFR:
                    if thirdparty.dns.serial.Serial(soa_rrset[0].serial) <= serial:
                        #
            ...
            for rrset in r.answer[answer_index:]:
                if done:
                    raise thirdparty.dns.exception.FormError("answers after final SOA")
                if rrset.rdtype == thirdparty.dns.rdatatype.SOA and rrset.name == oname:
                    if expecting_SOA:
                        if rrset[0].serial != serial:
                            raise thirdparty.dns.exception.FormError(
                                "IXFR base serial mismatch")
                        expecting_SOA = False
                    elif rdtype == thirdparty.dns.rdatatype.IXFR:
                    ...
                    if rrset == soa_rrset and \
                            (rdtype == thirdparty.dns.rdatatype.AXFR or
                             (rdtype == thirdparty.dns.rdatatype.IXFR and delete_mode)):
                        done = True
                elif expecting_SOA:
                    #
                    ...
                    # AXFR response.
                    #
                    rdtype = thirdparty.dns.rdatatype.AXFR
                    expecting_SOA = False
            if done and q.keyring and not r.had_tsig:
                raise thirdparty.dns.exception.FormError("missing TSIG")
            yield r
