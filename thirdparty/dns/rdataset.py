...
class Rdataset(thirdparty.dns.set.Set):
    ...
    def __init__(self, rdclass, rdtype, covers=thirdparty.dns.rdatatype.NONE, ttl=0):
    ...
    def add(self, rd, ttl=None):
        ...
        *rd*, a ``thirdparty.dns.rdata.Rdata``, the rdata

        *ttl*, an ``int``, the TTL.

        Raises ``thirdparty.dns.rdataset.IncompatibleTypes`` if the type and class
        do not match the type and class of the rdataset.

        Raises ``thirdparty.dns.rdataset.DifferingCovers`` if the type is a signature
        ...
        if self.rdtype == thirdparty.dns.rdatatype.RRSIG or \
           self.rdtype == thirdparty.dns.rdatatype.SIG:
            covers = rd.covers()
            if len(self) == 0 and self.covers == thirdparty.dns.rdatatype.NONE:
                self.covers = covers
    ...
    def update(self, other):
        ...
        *other*, a ``thirdparty.dns.rdataset.Rdataset``, the rdataset from which
        to update.
        """
    ...
    def __repr__(self):
        ...
        return '<DNS ' + thirdparty.dns.rdataclass.to_text(self.rdclass) + ' ' + \
               thirdparty.dns.rdatatype.to_text(self.rdtype) + ctext + \
    ...
    def to_text(self, name=None, origin=None, relativize=True,
        ...
        See ``thirdparty.dns.name.Name.choose_relativity`` for more information
        ...
        *name*, a ``thirdparty.dns.name.Name``.  If name is not ``None``, emit RRs with
        *name* as the owner name.

        *origin*, a ``thirdparty.dns.name.Name`` or ``None``, the origin for relative
        names.
    ...
    def to_wire(self, name, file, compress=None, origin=None,
        ...
        *name*, a ``thirdparty.dns.name.Name`` is the owner name to use.
...
def from_text_list(rdclass, rdtype, ttl, text_rdatas, idna_codec=None):
    ...
    *idna_codec*, a ``thirdparty.dns.name.IDNACodec``, specifies the IDNA
    ...
    Returns a ``thirdparty.dns.rdataset.Rdataset`` object.
    """
    ...
    for t in text_rdatas:
        rd = thirdparty.dns.rdata.from_text(r.rdclass, r.rdtype, t, idna_codec=idna_codec)
        r.add(rd)
    return r
