...
class HIP(thirdparty.dns.rdata.Rdata):
    ...
    @type servers: list of thirdparty.dns.name.Name objects
    @see: RFC 5205"""
    ...
    def from_text(cls, rdclass, rdtype, tok, origin=None, relativize=True):
        ...
        if len(hit) > 255:
            raise thirdparty.dns.exception.SyntaxError("HIT too long")
        ...
        while 1:
            ...
            if token.is_eol_or_eof():
                break
            server = thirdparty.dns.name.from_text(token.value, origin)
    ...
    def from_wire(cls, rdclass, rdtype, wire, current, rdlen, origin=None):
        ...
        servers = []
        while rdlen > 0:
            (server, cused) = thirdparty.dns.name.from_wire(wire[: current + rdlen],
                                                 current)
            current += cused
            rdlen -= cused
