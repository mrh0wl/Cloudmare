...
def _exponent_of(what, desc):
    ...
    if exp is None or exp < 0:
        raise thirdparty.dns.exception.SyntaxError("%s value out of bounds" % desc)
    return exp
...
def _decode_size(what, desc):
    exponent = what & 0x0F
    if exponent > 9:
        raise thirdparty.dns.exception.FormError("bad %s exponent" % desc)
    base = (what & 0xF0) >> 4
    if base > 9:
        raise thirdparty.dns.exception.FormError("bad %s base" % desc)
    return base * pow(10, exponent)


class LOC(thirdparty.dns.rdata.Rdata):
    ...
    def __init__(self, rdclass, rdtype, latitude, longitude, altitude,
        ...
        object.__setattr__(self, 'latitude', thirdparty.dns.rdata._constify(latitude))
        if isinstance(longitude, int):
            longitude = float(longitude)
        if isinstance(longitude, float):
            longitude = _float_to_tuple(longitude)
        object.__setattr__(self, 'longitude', thirdparty.dns.rdata._constify(longitude))
    ...
    def from_text(cls, rdclass, rdtype, tok, origin=None, relativize=True,
        ...
        if latitude[0] > 90:
            raise thirdparty.dns.exception.SyntaxError('latitude >= 90')
        t = tok.get_string()
        if t.isdigit():
            latitude[1] = int(t)
            if latitude[1] >= 60:
                raise thirdparty.dns.exception.SyntaxError('latitude minutes >= 60')
            t = tok.get_string()
            if '.' in t:
                ...
                if latitude[2] >= 60:
                    raise thirdparty.dns.exception.SyntaxError('latitude seconds >= 60')
                l = len(milliseconds)
        ...
        if t == 'S':
            ...
            raise thirdparty.dns.exception.SyntaxError('bad latitude hemisphere value')

        longitude[0] = tok.get_int()
        if longitude[0] > 180:
            raise thirdparty.dns.exception.SyntaxError('longitude > 180')
        t = tok.get_string()
        if t.isdigit():
            longitude[1] = int(t)
            if longitude[1] >= 60:
                raise thirdparty.dns.exception.SyntaxError('longitude minutes >= 60')
            t = tok.get_string()
            if '.' in t:
                (seconds, milliseconds) = t.split('.')
                if not seconds.isdigit():
                ...
                if longitude[2] >= 60:
                    raise thirdparty.dns.exception.SyntaxError('longitude seconds >= 60')
                l = len(milliseconds)
                if l == 0 or l > 3 or not milliseconds.isdigit():
                    raise thirdparty.dns.exception.SyntaxError(
        ...
        if t == 'W':
            ...
            raise thirdparty.dns.exception.SyntaxError('bad longitude hemisphere value')
    ...
    def from_wire_parser(cls, rdclass, rdtype, parser, origin=None):
        ...
        if latitude < _MIN_LATITUDE or latitude > _MAX_LATITUDE:
            raise thirdparty.dns.exception.FormError("bad latitude")
        ...
        if longitude < _MIN_LONGITUDE or longitude > _MAX_LONGITUDE:
            raise thirdparty.dns.exception.FormError("bad longitude")
