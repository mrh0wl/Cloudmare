...
class TSIG(thirdparty.dns.rdata.Rdata):
    ...
    def __init__(self, rdclass, rdtype, algorithm, time_signed, fudge, mac,
        ...
        *algorithm*, a ``thirdparty.dns.name.Name``.
        ...
        *other*, a ``bytes``
        ...
        object.__setattr__(self, 'mac', thirdparty.dns.rdata._constify(mac))
        object.__setattr__(self, 'original_id', original_id)
        object.__setattr__(self, 'error', error)
        object.__setattr__(self, 'other', thirdparty.dns.rdata._constify(other))

    def to_text(self, origin=None, relativize=True, **kw):
        algorithm = self.algorithm.choose_relativity(origin, relativize)
        return f"{algorithm} {self.fudge} {self.time_signed} " + \
               f"{len(self.mac)} {thirdparty.dns.rdata._base64ify(self.mac, 0)} " + \
               f"{self.original_id} {self.error} " + \
               f"{len(self.other)} {thirdparty.dns.rdata._base64ify(self.other, 0)}"
