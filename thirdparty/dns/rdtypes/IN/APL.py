...
class APLItem(object):
    ...
    def to_wire(self, file):
        if self.family == 1:
            address = thirdparty.dns.inet.inet_pton(thirdparty.dns.inet.AF_INET, self.address)
        elif self.family == 2:
            address = thirdparty.dns.inet.inet_pton(thirdparty.dns.inet.AF_INET6, self.address)
        else:
...
class APL(thirdparty.dns.rdata.Rdata):
    """APL record.
    ...
    def from_wire(cls, rdclass, rdtype, wire, current, rdlen, origin = None):
        ...
        while 1:
            ...
            if header[0] == 1:
                if l < 4:
                    address += '\x00' * (4 - l)
                address = thirdparty.dns.inet.inet_ntop(thirdparty.dns.inet.AF_INET, address)
            elif header[0] == 2:
                if l < 16:
                    address += '\x00' * (16 - l)
                address = thirdparty.dns.inet.inet_ntop(thirdparty.dns.inet.AF_INET6, address)
            else:
                #
