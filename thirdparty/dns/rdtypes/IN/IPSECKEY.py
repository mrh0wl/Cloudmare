...
class IPSECKEY(thirdparty.dns.rdata.Rdata):
    ...
    def __init__(self, rdclass, rdtype, precedence, gateway_type, algorithm,
        ...
        if gateway_type == 0:
            ...
            # check that it's OK
            thirdparty.dns.inet.inet_pton(thirdparty.dns.inet.AF_INET, gateway)
        elif gateway_type == 2:
            # check that it's OK
            thirdparty.dns.inet.inet_pton(thirdparty.dns.inet.AF_INET6, gateway)
        elif gateway_type == 3:
            pass
        else:
            raise SyntaxError(
    ...
    def to_wire(self, file, compress=None, origin=None):
        ...
        if self.gateway_type == 0:
            ...
            file.write(thirdparty.dns.inet.inet_pton(thirdparty.dns.inet.AF_INET, self.gateway))
        elif self.gateway_type == 2:
            file.write(thirdparty.dns.inet.inet_pton(thirdparty.dns.inet.AF_INET6, self.gateway))
        elif self.gateway_type == 3:
    ...
    def from_wire(cls, rdclass, rdtype, wire, current, rdlen, origin=None):
        ...
        if gateway_type == 0:
            ...
            gateway = thirdparty.dns.inet.inet_ntop(thirdparty.dns.inet.AF_INET,
                                         wire[current: current + 4])
            current += 4
            rdlen -= 4
        elif gateway_type == 2:
            gateway = thirdparty.dns.inet.inet_ntop(thirdparty.dns.inet.AF_INET6,
                                         wire[current: current + 16])
            current += 16
            rdlen -= 16
        elif gateway_type == 3:
            (gateway, cused) = thirdparty.dns.name.from_wire(wire[: current + rdlen],
                                                  current)
            current += cused
            rdlen -= cused
        else:
            raise thirdparty.dns.exception.FormError('invalid IPSECKEY gateway type')
