...
class PX(thirdparty.dns.rdata.Rdata):
    ...
    @type map822: thirdparty.dns.name.Name object
    @ivar mapx400: the mapx400 name
    @type mapx400: thirdparty.dns.name.Name object
    @see: RFC 2163"""
    ...
    def from_wire(cls, rdclass, rdtype, wire, current, rdlen, origin=None):
        ...
        current += 2
        rdlen -= 2
        (map822, cused) = thirdparty.dns.name.from_wire(wire[: current + rdlen],
                                             current)
        if cused > rdlen:
        ...
        (mapx400, cused) = thirdparty.dns.name.from_wire(wire[: current + rdlen],
                                              current)
        if cused != rdlen:
            raise thirdparty.dns.exception.FormError
        if origin is not None:
