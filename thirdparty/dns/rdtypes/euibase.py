...
class EUIBase(thirdparty.dns.rdata.Rdata):
    ...
    def __init__(self, rdclass, rdtype, eui):
        ...
        if len(eui) != self.byte_len:
            raise thirdparty.dns.exception.FormError('EUI%s rdata has to have %s bytes'
                                          % (self.byte_len * 8, self.byte_len))
        object.__setattr__(self, 'eui', eui)

    def to_text(self, origin=None, relativize=True, **kw):
        return thirdparty.dns.rdata._hexify(self.eui, chunksize=2).replace(' ', '-')
    ...
    def from_text(cls, rdclass, rdtype, tok, origin=None, relativize=True,
        ...
        for i in range(2, cls.byte_len * 3 - 1, 3):
            if text[i] != '-':
                raise thirdparty.dns.exception.SyntaxError('Dash expected at position %s'
                                                % i)
        text = text.replace('-', '')
        try:
            data = binascii.unhexlify(text.encode())
        except (ValueError, TypeError) as ex:
            raise thirdparty.dns.exception.SyntaxError('Hex decoding error: %s' % str(ex))
