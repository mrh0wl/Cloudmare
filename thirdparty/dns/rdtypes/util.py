...
class Gateway:
    ...
    def check(self):
        if self.type == 0:
            ...
            # check that it's OK
            thirdparty.dns.ipv4.inet_aton(self.gateway)
        elif self.type == 2:
            # check that it's OK
            thirdparty.dns.ipv6.inet_aton(self.gateway)
        elif self.type == 3:
            if not isinstance(self.gateway, thirdparty.dns.name.Name):
    ...
    def from_text(self, tok, origin=None, relativize=True, relativize_to=None):
        if self.type in (0, 1, 2):
            ...
            raise thirdparty.dns.exception.SyntaxError(self._invalid_type())

    def to_wire(self, file, compress=None, origin=None, canonicalize=False):
        if self.type == 0:
            pass
        elif self.type == 1:
            file.write(thirdparty.dns.ipv4.inet_aton(self.gateway))
        elif self.type == 2:
            file.write(thirdparty.dns.ipv6.inet_aton(self.gateway))
        elif self.type == 3:
    ...
    def from_wire_parser(self, parser, origin=None):
        if self.type == 0:
            ...
            return thirdparty.dns.ipv4.inet_ntoa(parser.get_bytes(4))
        elif self.type == 2:
            return thirdparty.dns.ipv6.inet_ntoa(parser.get_bytes(16))
        elif self.type == 3:
            return parser.get_name(origin)
        else:
            raise thirdparty.dns.exception.FormError(self._invalid_type())
...
class Bitmap:
            ...
            for (i, byte) in enumerate(bitmap):
                for j in range(0, 8):
                    if byte & (0x80 >> j):
                        ...
                        bits.append(thirdparty.dns.rdatatype.to_text(rdtype))
            text += (' ' + ' '.join(bits))
        return text
    ...
    def from_text(self, tok):
        ...
        while True:
            ...
            if rdtype == 0:
                raise thirdparty.dns.exception.SyntaxError(f"{self.type_name} with bit 0")
    ...
    def from_wire_parser(self, parser):
        ...
        while parser.remaining() > 0:
            ...
            if window <= last_window:
                raise thirdparty.dns.exception.FormError(f"bad {self.type_name} bitmap")
            bitmap = parser.get_counted_bytes()
            if len(bitmap) == 0 or len(bitmap) > 32:
                raise thirdparty.dns.exception.FormError(f"bad {self.type_name} octets")
