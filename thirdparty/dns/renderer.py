...
class Renderer:
    ...
    def add_edns(self, edns, ednsflags, payload, options=None):
        ...
        ednsflags |= (edns << 16)
        opt = thirdparty.dns.message.Message._make_opt(ednsflags, payload, options)
    ...
    def add_tsig(self, keyname, secret, fudge, id, tsig_error, other_data,
        ...
        if isinstance(secret, thirdparty.dns.tsig.Key):
            key = secret
        else:
            key = thirdparty.dns.tsig.Key(keyname, secret, algorithm)
        tsig = thirdparty.dns.message.Message._make_tsig(keyname, algorithm, 0, fudge,
                                              b'', id, tsig_error, other_data)
        (tsig, _) = thirdparty.dns.tsig.sign(s, key, tsig[0], int(time.time()),
                                  request_mac)
    ...
    def add_multi_tsig(self, ctx, keyname, secret, fudge, id, tsig_error,
        ...
        tsig = thirdparty.dns.message.Message._make_tsig(keyname, algorithm, 0, fudge,
                                              b'', id, tsig_error, other_data)
        (tsig, ctx) = thirdparty.dns.tsig.sign(s, key, tsig[0], int(time.time()),
