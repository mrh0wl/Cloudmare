...
import thirdparty.dns.reversename
import thirdparty.dns.tsig

if sys.platform == 'win32':
    import winreg  # pragma: no cover

class NXDOMAIN(thirdparty.dns.exception.DNSException):
    ...
    def canonical_name(self):
        ...
        IN = thirdparty.dns.rdataclass.IN
        CNAME = thirdparty.dns.rdatatype.CNAME
        cname = None
        for qname in self.kwargs['qnames']:
            ...
            if cname is not None:
                return thirdparty.dns.name.from_text(cname)
    ...
    def qnames(self):
        ...
        Returns a list of ``thirdparty.dns.name.Name``.
        """
    ...
    def responses(self):
        ...
        Returns a dict mapping a ``thirdparty.dns.name.Name`` to a
        ``thirdparty.dns.message.Message``.
        """
...
class YXDOMAIN(thirdparty.dns.exception.DNSException):
    """The DNS query name is too long after DNAME substitution."""

# The definition of the Timeout exception has moved from here to the
# thirdparty.dns.exception module.  We keep thirdparty.dns.resolver.Timeout defined for
# backwards compatibility.

Timeout = thirdparty.dns.exception.Timeout


class NoAnswer(thirdparty.dns.exception.DNSException):
...
class NoNameservers(thirdparty.dns.exception.DNSException):
...
class NotAbsolute(thirdparty.dns.exception.DNSException):
    """An absolute domain name is required but a relative name was provided."""


class NoRootSOA(thirdparty.dns.exception.DNSException):
    """There is no SOA RR at the DNS root name. This should never happen!"""


class NoMetaqueries(thirdparty.dns.exception.DNSException):
    """DNS metaqueries are not allowed."""

class NoResolverConfiguration(thirdparty.dns.exception.DNSException):
    ...
    def __init__(self, qname, rdtype, rdclass, response, nameserver=None,
        ...
        for count in range(0, 15):
            try:
                ...
                if rdtype != thirdparty.dns.rdatatype.CNAME:
                    try:
                        crrset = response.find_rrset(response.answer,
                                                     qname,
                                                     rdclass,
                                                     thirdparty.dns.rdatatype.CNAME)
            ...
            while 1:
                ...
                try:
                    srrset = response.find_rrset(response.authority, qname,
                                                 rdclass, thirdparty.dns.rdatatype.SOA)
                    ...
                    try:
                        qname = qname.parent()
                    except thirdparty.dns.name.NoParent:
                        break
...
class Cache:
    ...
    def get(self, key):
        ...
        *key*, a ``(thirdparty.dns.name.Name, int, int)`` tuple whose values are the
        query name, rdtype, and rdclass respectively.

        Returns a ``thirdparty.dns.resolver.Answer`` or ``None``.
        """
    ...
    def put(self, key, value):
        ...
        *value*, a ``thirdparty.dns.resolver.Answer``, the answer.
        """
...
class _Resolution:
    """Helper class for thirdparty.dns.resolver.Resolver.resolve().
    ...
    This class is a "friend" to thirdparty.dns.resolver.Resolver and manipulates
    ...
    def __init__(self, resolver, qname, rdtype, rdclass, tcp,
        ...
        if isinstance(qname, str):
            qname = thirdparty.dns.name.from_text(qname, None)
        rdtype = thirdparty.dns.rdatatype.RdataType.make(rdtype)
        if thirdparty.dns.rdatatype.is_metatype(rdtype):
            raise NoMetaqueries
        rdclass = thirdparty.dns.rdataclass.RdataClass.make(rdclass)
        if thirdparty.dns.rdataclass.is_metaclass(rdclass):
            raise NoMetaqueries
        ...
        self.qname = thirdparty.dns.name.empty
        self.nameservers = []
        ...
        while len(self.qnames) > 0:
            ...
            if self.resolver.cache:
                ...
                answer = self.resolver.cache.get((self.qname,
                                                  thirdparty.dns.rdatatype.ANY,
                                                  self.rdclass))
                if answer is not None and \
                   answer.response.rcode() == thirdparty.dns.rcode.NXDOMAIN:
            ...
            # Build the request
            request = thirdparty.dns.message.make_query(self.qname, self.rdtype,
                                             self.rdclass)
    ...
    def query_result(self, response, ex):
        ...
        if ex:
            ...
            self.errors.append((self.nameserver, self.tcp_attempt, self.port,
                                ex, response))
            if isinstance(ex, thirdparty.dns.exception.FormError) or \
            ...
            elif isinstance(ex, thirdparty.dns.message.Truncated):
                if self.tcp_attempt:
        ...
        if rcode == thirdparty.dns.rcode.NOERROR:
            ...
            if answer.rrset is None and self.raise_on_no_answer:
                raise NoAnswer(response=answer.response)
            return (answer, True)
        elif rcode == thirdparty.dns.rcode.NXDOMAIN:
            ...
            if self.resolver.cache:
                answer = Answer(self.qname, thirdparty.dns.rdatatype.ANY,
                                thirdparty.dns.rdataclass.IN, response)
                self.resolver.cache.put((self.qname,
                                         thirdparty.dns.rdatatype.ANY,
                                         self.rdclass), answer)

            return (None, True)
        elif rcode == thirdparty.dns.rcode.YXDOMAIN:
            yex = YXDOMAIN()
            ...
            # rcode in it.
            #
            if rcode != thirdparty.dns.rcode.SERVFAIL or not self.resolver.retry_servfail:
                self.nameservers.remove(self.nameserver)
            self.errors.append((self.nameserver, self.tcp_attempt, self.port,
                                thirdparty.dns.rcode.to_text(rcode), response))
            return (None, False)
...
class Resolver:
    ...
    def reset(self):
        ...
        self.domain = \
            thirdparty.dns.name.Name(thirdparty.dns.name.from_text(socket.gethostname())[1:])
        if len(self.domain) == 0:
            self.domain = thirdparty.dns.name.root
        self.nameservers = []
        ...
        self.keyring = None
        self.keyname = None
        self.keyalgorithm = thirdparty.dns.tsig.default_algorithm
        self.edns = -1
        self.ednsflags = 0
        self.payload = 0
        self.cache = None
        ...
        with contextlib.ExitStack() as stack:
            ...
            for l in f:
                ...
                if tokens[0] == 'nameserver':
                    ...
                    self.domain = thirdparty.dns.name.from_text(tokens[1])
                elif tokens[0] == 'search':
                    for suffix in tokens[1:]:
                        self.search.append(thirdparty.dns.name.from_text(suffix))
    ...
    def _config_win32_domain(self, domain):  # pragma: no cover
        ...
        self.domain = thirdparty.dns.name.from_text(str(domain))
    ...
    def _config_win32_search(self, search):  # pragma: no cover
        ...
        for s in search_list:
            if s not in self.search:
                self.search.append(thirdparty.dns.name.from_text(s))
    ...
    def _get_qnames_to_try(self, qname, search):
        ...
        if qname.is_absolute():
            ...
            if len(qname) > 1 or not search:
                qnames_to_try.append(qname.concatenate(thirdparty.dns.name.root))
    ...
    def resolve(self, qname, rdtype=thirdparty.dns.rdatatype.A, rdclass=thirdparty.dns.rdataclass.IN,
        ...
        *search*, a ``bool`` or ``None``, determines whether the
        ...
        Raises ``thirdparty.dns.exception.Timeout`` if no answers could be found
            ...
            while not done:
                ...
                try:
                    if thirdparty.dns.inet.is_address(nameserver):
                        if tcp:
                            response = thirdparty.dns.query.tcp(request, nameserver,
                                                     timeout=timeout,
                                                     port=port,
                                                     source=source,
                                                     source_port=source_port)
                        else:
                            response = thirdparty.dns.query.udp(request,
                                                     nameserver,
                                                     timeout=timeout,
                                                     port=port,
                                                     source=source,
                        ...
                        response = thirdparty.dns.query.https(request, nameserver,
                                                   timeout=timeout)
    ...
    def query(self, qname, rdtype=thirdparty.dns.rdatatype.A, rdclass=thirdparty.dns.rdataclass.IN,
              tcp=False, source=None, raise_on_no_answer=True, source_port=0,
        ...
        further details.
        """
        warnings.warn('please use thirdparty.dns.resolver.Resolver.resolve() instead',
    ...
    def resolve_address(self, ipaddr, *args, **kwargs):
        ...
        *ipaddr*, a ``str``, the IPv4 or IPv6 address you want to get
        ...
        return self.resolve(thirdparty.dns.reversename.from_address(ipaddr),
    ...
    def use_tsig(self, keyring, keyname=None,
        ...
        The parameters are passed to ``thirdparty.dns.message.Message.use_tsig()``;
...
def resolve(qname, rdtype=thirdparty.dns.rdatatype.A, rdclass=thirdparty.dns.rdataclass.IN,
    ...
    See ``thirdparty.dns.resolver.Resolver.resolve`` for more information on the
    parameters.
    """
...
def query(qname, rdtype=thirdparty.dns.rdatatype.A, rdclass=thirdparty.dns.rdataclass.IN,
          tcp=False, source=None, raise_on_no_answer=True,
    ...
    further details.
    """
    warnings.warn('please use thirdparty.dns.resolver.resolve() instead',
                  DeprecationWarning, stacklevel=2)
...
def resolve_address(ipaddr, *args, **kwargs):
    ...
    See ``thirdparty.dns.resolver.Resolver.resolve_address`` for more information on the
    parameters.
    """
...
def zone_for_name(name, rdclass=thirdparty.dns.rdataclass.IN, tcp=False, resolver=None):
    ...
    *resolver*, a ``thirdparty.dns.resolver.Resolver`` or ``None``, the resolver to use.
    ...
    while 1:
        try:
            answer = resolver.resolve(name, thirdparty.dns.rdatatype.SOA, rdclass, tcp)
...
def _getaddrinfo(host=None, service=None, family=socket.AF_UNSPEC, socktype=0,
    ...
    if flags & socket.AI_NUMERICHOST != 0:
        ...
        # because thirdparty.dns.query.* needs to call getaddrinfo() for IPv6 scoping
    ...
    try:
        ...
        # IPv6 address.
        thirdparty.dns.inet.af_for_address(host)
        return _original_getaddrinfo(host, service, family, socktype,
                                     proto, flags)
    except Exception:
        pass
    ...
    try:
        if family == socket.AF_INET6 or family == socket.AF_UNSPEC:
            v6 = _resolver.resolve(host, thirdparty.dns.rdatatype.AAAA,
        ...
        if family == socket.AF_INET or family == socket.AF_UNSPEC:
            v4 = _resolver.resolve(host, thirdparty.dns.rdatatype.A,
                                   raise_on_no_answer=False)
            host = v4.qname
...
def _gethostbyaddr(ip):
    try:
        ...
        try:
            thirdparty.dns.ipv4.inet_aton(ip)
        except Exception:
            raise socket.gaierror(socket.EAI_NONAME,
                                  'Name or service not known')
        sockaddr = (ip, 80)
    ...
    bin_ip = thirdparty.dns.inet.inet_pton(family, ip)
    for item in tuples:
        addr = item[4][0]
        bin_addr = thirdparty.dns.inet.inet_pton(family, addr)
        if bin_ip == bin_addr:
