...
@type ipv4_reverse_domain: thirdparty.dns.name.Name object
@var ipv6_reverse_domain: The DNS IPv6 reverse-map domain, ip6.arpa.
@type ipv6_reverse_domain: thirdparty.dns.name.Name object
"""
...
def from_address(text):
    ...
    try:
        ...
        parts = ['%d' %
                 byte for byte in bytearray(thirdparty.dns.ipv4.inet_aton(text))]
        origin = ipv4_reverse_domain
    parts.reverse()
