...
def from_text(textring):
    ...
    pairs into a binary keyring which has (thirdparty.dns.name.Name, bytes) pairs, or
    a dictionary containing (textual DNS name, (algorithm, base64 secret))
    pairs into a binary keyring which has (thirdparty.dns.name.Name, thirdparty.dns.tsig.Key) pairs.
    @rtype: dict"""

    keyring = {}
    for (name, value) in textring.items():
        name = thirdparty.dns.name.from_text(name)
        if isinstance(value, str):
            keyring[name] = thirdparty.dns.tsig.Key(name, value).secret
        else:
            (algorithm, secret) = value
            keyring[name] = thirdparty.dns.tsig.Key(name, secret, algorithm)
    return keyring


def to_text(keyring):
    """Convert a dictionary containing (thirdparty.dns.name.Name, thirdparty.dns.tsig.Key) pairs
    into a text keyring which has (textual DNS name, (textual algorithm,
    base64 secret)) pairs, or a dictionary containing (thirdparty.dns.name.Name, bytes)
