...
class Zone:
    ...
    ``thirdparty.dns.name.Name object``, or it may be a string.  In either case,
    if the name is relative it is treated as relative to the origin of
    the zone.
    """

    node_factory = thirdparty.dns.node.Node

    __slots__ = ['rdclass', 'origin', 'nodes', 'relativize']

    def __init__(self, origin, rdclass=thirdparty.dns.rdataclass.IN, relativize=True):
        """Initialize a zone object.

        *origin* is the origin of the zone.  It may be a ``thirdparty.dns.name.Name``,
    ...
    def find_node(self, name, create=False):
        ...
        *name*: the name of the node to find.
        The value may be a ``thirdparty.dns.name.Name`` or a ``str``.  If absolute, the
        ...
        *create*, a ``bool``.  If true, the node will be created if it does
        ...
        Returns a ``thirdparty.dns.node.Node``.
        """
    ...
    def get_node(self, name, create=False):
        ...
        *create*, a ``bool``.  If true, the node will be created if it does
        ...
        Returns a ``thirdparty.dns.node.Node`` or ``None``.
        """
    ...
    def find_rdataset(self, name, rdtype, covers=thirdparty.dns.rdatatype.NONE,
        ...
        *covers*, an ``int`` or ``str`` or ``None``, the covered type.
        Usually this value is ``thirdparty.dns.rdatatype.NONE``, but if the
        rdtype is ``thirdparty.dns.rdatatype.SIG`` or ``thirdparty.dns.rdatatype.RRSIG``,
        ...
        *create*, a ``bool``.  If true, the node will be created if it does
        ...
        Returns a ``thirdparty.dns.rdataset.Rdataset``.
        """

        name = self._validate_name(name)
        rdtype = thirdparty.dns.rdatatype.RdataType.make(rdtype)
        if covers is not None:
            covers = thirdparty.dns.rdatatype.RdataType.make(covers)
        node = self.find_node(name, create)
        return node.find_rdataset(self.rdclass, rdtype, covers, create)

    def get_rdataset(self, name, rdtype, covers=thirdparty.dns.rdatatype.NONE,
                     create=False):
        """Look for an rdataset with the specified name and type in the zone.
        ...
        *create*, a ``bool``.  If true, the node will be created if it does
        ...
        Returns a ``thirdparty.dns.rdataset.Rdataset`` or ``None``.
        """
    ...
    def delete_rdataset(self, name, rdtype, covers=thirdparty.dns.rdatatype.NONE):
    ...
    def replace_rdataset(self, name, replacement):
        ...
        *replacement*, a ``thirdparty.dns.rdataset.Rdataset``, the replacement rdataset.
        """
    ...
    def find_rrset(self, name, rdtype, covers=thirdparty.dns.rdatatype.NONE):
        """Look for an rdataset with the specified name and type in the zone,
        ...
        *create*, a ``bool``.  If true, the node will be created if it does
        ...
        Returns a ``thirdparty.dns.rrset.RRset`` or ``None``.
        """
    ...
    def get_rrset(self, name, rdtype, covers=thirdparty.dns.rdatatype.NONE):
        """Look for an rdataset with the specified name and type in the zone,
        and return an RRset encapsulating it.
    ...
    def iterate_rdatasets(self, rdtype=thirdparty.dns.rdatatype.ANY,
                          covers=thirdparty.dns.rdatatype.NONE):
        """Return a generator which yields (name, rdataset) tuples for
        all rdatasets in the zone which have the specified *rdtype*
        and *covers*.  If *rdtype* is ``thirdparty.dns.rdatatype.ANY``, the default,
        ...
        *covers*, an ``int`` or ``str`` or ``None``, the covered type.
        ...
        for (name, node) in self.items():
            for rds in node:
                if rdtype == thirdparty.dns.rdatatype.ANY or \
                   (rds.rdtype == rdtype and rds.covers == covers):
                    yield (name, rds)

    def iterate_rdatas(self, rdtype=thirdparty.dns.rdatatype.ANY,
                       covers=thirdparty.dns.rdatatype.NONE):
    ...
    def check_origin(self):
        ...
        Raises ``thirdparty.dns.zone.NoSOA`` if there is no SOA RRset.

        Raises ``thirdparty.dns.zone.NoNS`` if there is no NS RRset.
...
class _MasterReader:
    ...
    def _rr_line(self):
        ...
        if ttl is None:
            raise thirdparty.dns.exception.SyntaxError("Missing default TTL value")
    ...
    def read(self):
        ...
        try:
            while 1:
                ...
                if token.is_eof():
                    ...
                    if c == '$TTL':
                        ...
                        self.default_ttl = thirdparty.dns.ttl.from_text(token.value)
                        self.default_ttl_known = True
                        self.tok.get_eol()
                    elif c == '$ORIGIN':
            ...
            if detail is None:
                detail = "syntax error"
            ex = thirdparty.dns.exception.SyntaxError(
...
def from_text(text, origin=None, rdclass=thirdparty.dns.rdataclass.IN,
    ...
    *origin*, a ``thirdparty.dns.name.Name``, a ``str``, or ``None``.  The origin
    ...
    *zone_factory*, the zone factory to use or ``None``.  If ``None``, then
    ``thirdparty.dns.zone.Zone`` will be used.  The value may be any class or callable
    that returns a subclass of ``thirdparty.dns.zone.Zone``.
    ...
    *idna_codec*, a ``thirdparty.dns.name.IDNACodec``, specifies the IDNA
    ...
    Returns a subclass of ``thirdparty.dns.zone.Zone``.
    """
    ...
    if filename is None:
        filename = '<string>'
    tok = thirdparty.dns.tokenizer.Tokenizer(text, filename, idna_codec=idna_codec)
...
def from_xfr(xfr, zone_factory=Zone, relativize=True, check_origin=True):
    ...
    *xfr*, a generator of ``thirdparty.dns.message.Message`` objects, typically
    ``thirdparty.dns.query.xfr()``.
